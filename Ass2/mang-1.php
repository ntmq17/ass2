...
if (isset($_POST['btn_goi'])) {
    $mang_so = explode(",", $_POST['nhap_mang']); /*Hàm explode() có chức năng chuyển 
    đổi một chuỗi thành một mảng và mỗi phần tử được tách bởi một chuỗi con hay một ký tự nào đó*/
    $n = count($mang_so);
    for ($i = 0; $i < $n; $i++) {
        $ket_qua += $mang_so[$i];
    }
